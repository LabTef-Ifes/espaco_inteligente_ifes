...
for it_frames in range(video_loader.n_frames()):
    ...
    # print(perna_esquerda_aux1,perna_esquerda_aux2,aux_left_knee_angle)
    ...
    flex_quadril_ang.append(aux_flex_quadril_ang)
    #print(aux_left_knee_angle)
    #print(perna_esquerda_aux1,perna_esquerda_aux2,aux_left_knee_angle)
    perna_esquerda.append(perna_esquerda_aux1)
    perna_direita_aux,dist_do_chao_aux,aux_largura_da_passada,aux_right_knee_angle,altura_pe_direito,altura_pe_esquerdo=Parameters.right_leg(localizations[it_frames])
    #print(altura_pe_direito,altura_pe_esquerdo)
    ...
    if ((altura_pe_esquerdo>=0.125 and altura_pe_direito<=0.125) or (altura_pe_esquerdo<=0.125 and altura_pe_direito>=0.125)):
        ...
        if(distance_feet[-1] > (Swing_real or Stance_real)):
            k=k+1
    ...
    if SkeletonsCoord.joint_coord(localizations[it_frames], 12) and SkeletonsCoord.joint_coord(localizations[it_frames], 15):
        ...
        distance_feet_3.append(np.sqrt((right_foot[0]-left_foot[0])**2+(right_foot[1]-left_foot[1])**2)) ### Plano XY
        #print(distance_feet_3[-1])
        instante.append(time.time() - tempo_inicial)
        # tempo_inicial
        #print(len(distance_feet))
        ...
        if len(distance_feet)>=3:
            #print((distance_feet[len(distance_feet)-1]),(distance_feet[len(distance_feet)-2]))
            ...
            #print(comprimento_swing[-1],comprimento_stance[-1], simetria_comprimento_passo[-1])
            
            if (contador_numero_de_passos%2)==0:
                quant_de_ciclos=contador_numero_de_passos/2
                #print("numero de ciclos:", quant_de_ciclos)
            ...
            if (quant_de_ciclos%slide_result)==0:
    ...
    ang_ext_quadril.append(aux_ang_ext_quadril)
    ...
    # print(aux_left_knee_angle,aux_flexion_left_knee)

    # print(left_knee_angle)
    # print(left_knee_angle(len(localizations[it_frames])))
    ...
    cv2.imshow('', display_image)
    cv2.putText(display_image, "Movimento detectado: %s" % output_neural_network, (1300,150),cv2.QT_FONT_NORMAL, .4, (100,100,10),1, cv2.LINE_AA) 
    cv2.imshow('', display_image)
...
ang_ext_quadril=ang_ext_quadril[1:]
...
tempo_total_em_min=tempo_total/60
cadencia=contador_numero_de_passos/tempo_total_em_min
print(len(tempo_passo),contador_numero_de_passos)
print(" %.3f s" % tempo_total)
print("Cadência (passos/min): %.3f " % cadencia)

tempo_total=time.time()-tempo_inicial # Tempo em segundos
tempo_total_em_min=tempo_total/60
print("Tempo total: %.3f s " % tempo_total)
cadencia=contador_numero_de_passos/tempo_total_em_min
print("Cadência 2 (passos/min): %.3f " % cadencia)
...
title='Ângulo de extensão joelho esquerdo durante a caminhada'
...
title='Ângulo de extensão joelho direito durante a caminhada'
...
ang_ext_quadril=Parameters.retira_primeiro_elemento(ang_ext_quadril)
title='Ângulo da extensão do quadril direito durante a caminhada'
Plota_graficos.plota_angulo_medido(ang_ext_quadril,title)
...
aux_angulo=Plota_graficos.plota_angulo_medido_normalizado(w,title)
...
flexion_left_knee_angle=Plota_graficos.plota_angulo_medido_normalizado(l,title)
...
flexion_right_knee_angle=Plota_graficos.plota_angulo_medido_normalizado(z,title)
...
right_ang_ext_quadril=Plota_graficos.plota_angulo_medido_normalizado(n,title)
...
left_ang_ext_quadril=Plota_graficos.plota_angulo_medido_normalizado(lu,title)
...
velocidade_angular_flexion_right_knee_angle=Plota_graficos.plota_angulo_medido_normalizado(iza,title)

#print("Média:")
#print(statistics.mean(flexion_left_knee_angle))
Plota_graficos.plota_grafico(flex_quadril_ang,"Ângulo de extensão do quadril esquerdo ")
...
#print("%s" % output_neural_network)     
#Plota_graficos.plota_grafico(flex_quadril_ang)
vid.release()
