...
def render_skeletons_3d(ax, skeletons, links, colors, posicao_inicial, final_passo, pe_id, pe_direito, pe_esquerdo, posicao_final, i):
    """_summary_
    ...
    for skeleton in skeletons_pb.objects:
        ...
        if pe_esquerdo[0] < abs(0.09) and pe_direito[0] >= abs(0.09):
            posicao_inicial = pe_esquerdo
            final_passo = False
            pe_id = 12

        if pe_direito[2] < abs(0.09) and pe_esquerdo[2] >= abs(0.09):
        ...
        if pe_direito[2] < abs(0.09) and pe_esquerdo[2] < abs(0.09):
            if pe_id == 12:
                ...
                posicao_inicial = pe_esquerdo
        if final_passo:
            comprimento_passo = abs(np.sqrt(
                (posicao_final[0] - posicao_inicial[0])**2 + (posicao_final[1] - posicao_inicial[1])**2))
...
for it_frames in range(video_loader.n_frames()):
    ...
    posicao_inicial, posicao_final, final_passo, comprimento_passo, pe_id, pe_direito, pe_esquerdo, posicao_final = render_skeletons_3d(
        ax, localizations[it_frames], links, colors, posicao_inicial, final_passo, pe_id, pe_direito, pe_esquerdo, posicao_final, i
    )
    comprimento_passo_vetor.append(comprimento_passo)
    i = i+1
    fig.canvas.draw()
...
log.info("Comprimento de passo: {}".format(comprimento_passo))
print("Média do comprimento de passos: %5.3f" %
      statistics.mean(comprimento_passo_vetor))
print("Desvio padrão das medições do comprimento de passos: %5.3f" %
      statistics.pstdev(comprimento_passo_vetor))
# tempo_duplo_suporte=0.2*(tempo_final-tempo_inicial)
#log.info("Tempo total: " + str(tempo_duplo_suporte))
log.info('Exiting')
