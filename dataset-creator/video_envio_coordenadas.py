...
# cam_id, porcentagem, tempo_total_em_min não usados ???
def file_maker(cam_id, juntas, perdidas, porcentagem, porcentagem_3d, perda_media, variancia, y, x, perna_esquerda,
               perna_direita, maior_passo_medido, tempo_total, velocidade_media, passos_por_min, contador,
               tempo_total_em_min):
    """
    Args:
        cam_id:
    ...
    for cam_id in range(0, 4):
        ...
        file_results.write(
            "cam{}: Juntas detectadas: {} | Perdidas: {} |  {:.2f} %".format(cam_id, juntas[cam_id], perdidas[cam_id],
                                                                             porcentagem))
        file_results.write("\n")

    file_results.write(
        "Juntas detectadas [Serviço 3d]: {} | Perdidas: {} |  {:.2f} %".format(juntas_3d, perdidas_3d, porcentagem_3d))
    file_results.write("\n")
...
for cam_id in range(0, 4):
    porcentagem = (perdidas[cam_id] / juntas[cam_id]) * 100
    log.info("cam{}: Juntas detectadas: {} | Perdidas: {} |  {:.2f} %".format(cam_id, juntas[cam_id], perdidas[cam_id],
                                                                              porcentagem))
...
file_maker(cam_id, juntas, perdidas, porcentagem, porcentagem_3d, perda_media, variancia, y, x, perna_esquerda,
           perna_direita, maior_passo_medido, tempo_total, velocidade_media, passos_por_min, contador,
           tempo_total_em_min)
