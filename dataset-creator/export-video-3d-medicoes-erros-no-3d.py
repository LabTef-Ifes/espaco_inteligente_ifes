...
for it_frames in range(video_loader.n_frames()):
        ...
        if len(distance_feet) >= 3:
            ...
            if (contador_numero_de_passos % 2) == 0:
                ...
                comprimento_passo_medido.append(
                    (picos_distancia[contador_numero_de_passos-1]+picos_distancia[contador_numero_de_passos-2]))  # Conta cada passada dada
    ...
    cv2.putText(display_image, "Angulo de caminhada: %.3f graus" % aux_angulo[it_frames] + " +- %.3f" % (statistics.mean(
        aux_angulo) - aux_angulo[it_frames]), (1300, 30), cv2.FONT_HERSHEY_SIMPLEX, .4, (100, 00, 10), 1, cv2.LINE_AA)
    cv2.imshow('', display_image)
    Velocidade_no_instante = (
        abs(distance_feet_2[len(distance_feet_2)-1])/(time.time()-aux_tempo))
    cv2.putText(display_image, "Velocidade da caminhada: %.3f m/s " % Velocidade_no_instante + " +- %.3f" % (statistics.mean(
        aux_velocidade_instante)-Velocidade_no_instante), (1300, 45), cv2.FONT_HERSHEY_SIMPLEX, .4, (100, 00, 10), 1, cv2.LINE_AA)
    cv2.imshow('', display_image)
    ...
    cv2.imshow('', display_image)
    cv2.putText(display_image, "Stance/Swing: %.3f m" % abs(distance_feet_2[len(distance_feet_2)-1]) + " +- %.3f" % abs(
        (distance_feet_2[len(distance_feet_2)-1])-Stance_real), (1300, 90), cv2.FONT_HERSHEY_SIMPLEX, .4, (100, 00, 10), 1, cv2.LINE_AA)
    cv2.imshow('', display_image)
...
flexion_left_knee_angle = Parameters.normaliza_vetor(
    flexion_left_knee_angle, quant_de_ciclos, quant_de_ciclos_desejado, 70)  # pico do sinal em 70 % do ciclo para a flexão
...
Parameters.file_maker(cam_id, juntas, perdidas, juntas_3d, perdidas_3d, average_height, idade, porcentagem, porcentagem_3d, perda_media, variancia, y, x, perna_esquerda, perna_direita, maior_passo_medido, tempo_total, velocidade_media, cadencia, contador_numero_de_passos, tempo_total_em_min, dist_do_chao, comprimento_passo_real_medido, Stance_real,
                      Swing_real, distance_feet, dist_dos_pes_inicial, picos_distancia, comprimento_passo_medido, comprimento_swing, comprimento_stance, aux_angulo, altura_quadril, coxa_perna_esquerda, angulo_real_joelho_esquerdo, comprimento_passo_real_medido, flexion_left_knee_angle, simetria_comprimento_passo, largura_da_passada, ang_ext_quadril_direito)
Parameters.erro_medio_da_caminhada(comprimento_passo_real_medido, Stance_real, Swing_real, distance_feet, dist_dos_pes_inicial, picos_distancia, comprimento_passo_medido, comprimento_swing, comprimento_stance, aux_angulo, altura_quadril,
                                   perna_direita, coxa_perna_esquerda, angulo_real_joelho_esquerdo, flexion_left_knee_angle, flexion_right_knee_angle, simetria_comprimento_passo, largura_da_passada, left_extension_hip_angle, right_extension_hip_angle)
