...
"""Acessa o tópico de comunicação das câmeras e consome suas imagens
"""
N_CONCURRENT_TAKS = 4
...
def place_images(output_image, images_):
    """Cursed function to resize four images into 1 image
...
def write_image_to_file(filename: str, content: str) -> None:
...
def main(executor:ThreadPoolExecutor):
    ...
    while True:
        ...
        content = message.unpack()
        print(f"[{datetime.now()}] Message received from topic {message.topic}")
        executor.submit(write_image_to_file, f"{msgs_count:08d}_{message.topic}", content)
        msgs_count += 1

if __name__ == "__main__":
   with ThreadPoolExecutor(max_workers=N_CONCURRENT_TAKS) as exec:
       main(exec)
